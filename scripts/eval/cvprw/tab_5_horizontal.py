...
set_gray_grid(ax, "Top View", "X [cm]", "-Z [cm]", (-150, 150), (-20, 250), fontsize_axis, font)
...
for step, data_dict in enumerate(loader):
    ...
    predicted_x_dir = np.sign(predicted_x)  # +1 (right), -1 (left), 0 (no movement) 
    actual_x_dir = np.sign(actual_x) 

    # Compare predictions with ground truth
    color = "silver"
    if predicted_x_dir == actual_x_dir:
...
for recording_id, (x_ball, y_ball, z_ball, colors) in recording_lines.items():
    #smoothed_predictions = gaussian_filter1d(predictions, sigma=4) 
    for i in range(1, len(x_ball)):
        ...
        if (rec1==None and colors[i]=="mediumseagreen"):
            ...
            label = "Wrong" 
            rec2=1  
        ax.plot(y_ball[i-1:i+1], x_ball[i-1:i+1], color=colors[i], linewidth=3, alpha=0.75, label=label) 

    # Smoothed prediction line (thicker, blue)
    # ax.plot(times, smoothed_predictions, color="royalblue", linewidth=2.5, alpha=0.9, 
    #          label="Smoothed Prediction" if i == 1 else "")
...
if dataset_name == "test":
    ax.legend(loc="upper left", fontsize=fontsize_axis, frameon=True) 
plt.tight_layout()
plt.savefig(os.path.join(base_path, f"top_place_of_collision_plot_{model_name}_{dataset_name}.pdf"))
