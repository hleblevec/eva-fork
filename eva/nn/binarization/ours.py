...
class BiTestConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, bias=False, dilation=0, groups=1):
    ...
    def forward(self, x): 
        ...
        real_weights = real_weights - real_weights.view(real_weights.size(0), -1).mean(-1).view(real_weights.size(0), 1, 1, 1) # ir-net
...
class BiTestLinear(nn.Linear):
    def __init__(self,  in_channels, out_channels, bias=True, binary_act=True):
    ...
    def forward(self, x):
        ...
        # Get real weights
        real_weights = self.weight
        real_weights = real_weights - real_weights.mean(-1).view(-1, 1) # ir-net
